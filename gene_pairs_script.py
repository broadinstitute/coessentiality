...
def calculate_pairs(start_index, end_index, chuck_num, screens = screens):
    ...
    np.save('data/pairs/GLS_p_{}.npy'.format(chuck_num), GLS_p)
    np.save('data/pairs/GLS_sign_{}.npy'.format(chuck_num), GLS_sign)
    subset_screens.index.to_series().to_csv('data/pairs/genes_{}.txt'.format(chuck_num), index=False, header=False)
